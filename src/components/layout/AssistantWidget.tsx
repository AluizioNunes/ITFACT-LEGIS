...
export function AssistantWidget() {
    ...
    const getPageTitle = React.useCallback((path: string) => {
    ...
    }, [pathname, user?.name, getPageTitle]);
        ...
        try {
            ...
            if (!res.ok) {
                ...
                const aiResponse: Message = {
                    ...
                    content: errorData.detail || "Desculpe, não consegui conectar à base de conhecimento no momento.",
                    senderId: 'ai',
                    scope: 'AI',
                    sender: { name: 'ASSISTENTE LEGIS' },
                    createdAt: new Date().toISOString()
            ...
            const errorMsg: Message = {
                id: (Date.now() + 1).toString(),
                content: "Erro Crítico: Não foi possível alcançar o servidor de IA. Verifique sua conexão.",
                senderId: 'ai',
                scope: 'AI',
                sender: { name: 'ASSISTENTE LEGIS' },
                createdAt: new Date().toISOString()
            ...
            <AnimatePresence>
                {isOpen && !isMinimized && (
                    <motion.div
                        ...
                        className="mb-4 w-[480px] max-w-[90vw] h-[min(700px,80vh)] min-h-[500px] origin-bottom-right"
                    >
                        <Card className="h-full flex flex-col shadow-[0_25px_60px_rgba(30,58,138,0.25)] border-blue-200/50 bg-white overflow-hidden rounded-3xl">
                            <CardHeader className="p-4 bg-blue-600 text-white flex flex-row items-center justify-between shrink-0 shadow-md relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <CardTitle className="text-xs font-bold tracking-wider uppercase">ASSISTENTE LEGIS</CardTitle>
                                        <span className="text-[8px] font-medium text-blue-100 uppercase opacity-70">Câmara Municipal</span>
                                    </div>
                                </div>
                                <div className="relative z-10 flex items-center gap-1.5">
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-white hover:bg-white/15 rounded-lg transition-colors" onClick={() => setIsMinimized(true)}>
                                        <Minimize2 className="h-3.5 w-3.5" />
                                    </Button>
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-white hover:bg-white/25 rounded-lg transition-colors" onClick={() => setIsOpen(false)}>
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
                                <TabsList className="grid w-full grid-cols-2 h-11 rounded-none bg-slate-50 border-b p-1 uppercase">
                                    <TabsTrigger value="AI" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-700 transition-all flex gap-2 text-[9px] font-bold tracking-widest leading-none">
                                        <Bot className="h-3.5 w-3.5 text-blue-500" /> AJUDA IA
                                    </TabsTrigger>
                                    <TabsTrigger value="TEAM" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-700 transition-all flex gap-2 text-[9px] font-bold tracking-widest leading-none">
                                        <Users className="h-3.5 w-3.5 text-blue-500" /> CHAT EQUIPE
                                    </TabsTrigger>
                                </TabsList>

                                <TabsContent
                                    value="AI"
                                    className="flex-grow flex flex-col m-0 p-0 overflow-hidden bg-slate-50/20 data-[state=active]:flex min-h-0"
                                >
                                    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                                        ...
                                        {suggestions.length > 0 && (
                                            <div className="shrink-0 px-5 py-2 flex gap-2 overflow-x-auto no-scrollbar bg-blue-50/30 border-t border-blue-100/50">
                                                {suggestions.map((s, i) => (
                                                    <button
                                                        key={i}
                                    ...
                                    <div className="shrink-0 border-t bg-white p-4">
                                        <div className="flex gap-2 p-1.5 rounded-xl border border-blue-100 bg-blue-50/20 shadow-sm focus-within:ring-2 ring-blue-500/10 focus-within:border-blue-400/40 transition-all">
                                            <Input
                                ...
                                >
                                    ...
                                    <div className="shrink-0 border-t bg-white p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.02)]">
                                        <div className="flex gap-2 p-1.5 rounded-xl border border-blue-100 bg-blue-50/20 shadow-sm focus-within:ring-2 ring-blue-500/10 transition-all">
                                            <Input
                                                placeholder={selectedUser ? `MENSAGEM PRIVADA PARA ${selectedUser.name.split(' ')[0]}...` : "Mensagem para a equipe..."}
                                                className="border-none bg-transparent shadow-none font-medium text-[13px] placeholder:text-blue-400/50 h-9"
                                                value={chatInput}
                                                onChange={(e) => setChatInput(e.target.value)}
                                                onKeyDown={(e) => e.key === 'Enter' && sendChatMessage()}
                                            />
                                            <Button size="icon" className="shrink-0 h-9 w-9 rounded-lg bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-200 transition-all" onClick={sendChatMessage}>
                                                <Send className="h-4 w-4" />
                                            </Button>
                                        </div>
                                    </div>
